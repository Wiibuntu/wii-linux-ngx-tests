...
static int sthcd_driver_probe(struct device *dev)
{
	struct sthcd_hcd *sthcd;
	struct usb_hcd *hcd;
	int error = -ENOMEM;
...
static int sthcd_driver_remove(struct device *dev)
{
...
static int __exit sthcd_of_remove(struct platform_device *odev)
{
...
static struct platform_driver sthcd_of_driver = {
	.driver = {
		.name = DRV_MODULE_NAME,
		.owner = THIS_MODULE,
		.of_match_table = sthcd_of_match,
	},
...
static int __init sthcd_module_init(void)
{
	...
	return platform_driver_register(&sthcd_of_driver);
}

static void __exit sthcd_module_exit(void)
{
	platform_driver_unregister(&sthcd_of_driver);
}
